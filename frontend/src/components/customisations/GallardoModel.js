...
import gallardoModel from '../../models3d/gallardo/scene.glb';

const GallardoModel = memo(({ canvasContainerRef, carChanges }) => {
  ...
  useEffect(() => {
    ...
    let cameraFar = isDesktop ? 5 : (isLaptop ? 6 : 9);
    ...
    loader.load(gallardoModel, (gltf) => {
      ...
      theModel.current.rotation.y = 0.3 * Math.PI;
      isLaptop ? theModel.current.position.x = 0 : theModel.current.position.x = 0.15;
    ...
    const dirLight = new THREE.DirectionalLight(0xffffff, 19);
    ...
    scene.add(dirLight2);
    const dirLight3 = new THREE.DirectionalLight(0xffffff, 14);
