...
import urusModel from '../../models3d/urus/scene.glb';

const UrusModel = memo(({ canvasContainerRef, carChanges }) => {
  ...
  useEffect(() => {
    ...
    loader.load(urusModel, (gltf) => {
      ...
      isDesktop ? theModel.current.scale.set(1, 1, 1) : (isLaptop ? theModel.current.scale.set(0.8, 0.8, 0.8) : theModel.current.scale.set(0.55, 0.55, 0.55));
      theModel.current.rotation.y = 1.75 * Math.PI;
      theModel.current.position.y = -1;
      isDesktop ? theModel.current.position.x = 50 : theModel.current.position.x = 20;
